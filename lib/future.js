...
const emit = function ($item, item) {
  ...
  if (item.context?.length > 0 || (isSecureContext && !networkSecurity.isLoopbackHost(location.hostname))) {
    $item.append(`\
...
`)
  ...
  if (isSecureContext && offerAltLineup && !networkSecurity.isLoopbackHost(location.hostname)) {
