...
const emit = function ($item, item) {
  ...
  if (wiki.asSlug(item.title) !== proposedSlug) {
    $item.append(
      "<p style='font-weight: 500;'>Page titles with leading/trailing spaces cannot be used to create a new page.</p>",
    )
  } else {
  ...
  if ((transport = item.create?.source?.transport)) {
    ...
    if (!networkSecurity.shouldBlockLoopbackTarget('localhost')) {
  ...
  if ((info = neighborhood.sites[location.host]) && info.sitemap) {
    for (var localPage of info.sitemap) {
      if (localPage.slug.match(/-template$/)) {
        $item.append(
          `<br><button class="create" data-slug=${localPage.slug}>create</button> from ${resolve.resolveLinks(`[[${localPage.title}]]`)}`,
        )
      }
    }
  }

  if (item.context?.length > 0 || (location.protocol === 'https:' && !networkSecurity.isLoopbackHost(location.hostname))) {
    $item.append(`\
...
`)
  ...
  if (location.protocol === 'https:' && offerAltLineup && !networkSecurity.isLoopbackHost(location.hostname)) {
...
const bind = ($item, item) =>
  $item.find('button.transport').on('click', () => {
