...
const pushToLocal = function ($page, pagePutInfo, action) {
  ...
  if (action.type === 'create') {
    ...
    page.journal ||= []
    if ((site = action['fork'])) {
      if (!networkSecurity.shouldStripLoopbackProvenance(site)) {
...
pageHandler.put = function ($page, action) {
  ...
  if (networkSecurity.shouldStripLoopbackProvenance(forkFrom)) {
    forkFrom = null
  }
  ...
  if (action.type === 'fork' && networkSecurity.shouldStripLoopbackProvenance(action.site)) {
    delete action.site
  }
