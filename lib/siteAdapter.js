  ...
  return testWikiSite(
    ...
    function () {
      switch (location.protocol) {
        case 'http:':
          // When running on http origins, browsers can still enforce HTTPS-Only mode
          // and upgrade http://... requests, breaking HTTP-only sites.
          // Also, some sites block cross-origin XHR without CORS headers.
          // Try the same-origin proxy first; it will fall back to http server-side.
  ...
  return {
    ...
    get(route, callback) {
      ...
      if (sitePrefix[site] != null) {
        ...
        // don't yet know how to fetch from site, so find adapter and proceed
        //findAdapterQ.push {site: site}, (prefix) ->
        findAdapter(site, function (prefix) {
          if (prefix === '') {
            done({ msg: `${site} is unreachable` }, null)
    ...
    put(route, data, done) {
      // console.log(`wiki.site(${site}).put(${route}, ${JSON.stringify(data)})`)
      if (sitePrefix[site] != null) {
        if (sitePrefix[site] === '') {
    ...
    delete(route, done) {
      // console.log(`wiki.site(${site}).delete(${route})`)
      if (sitePrefix[site] != null) {
        if (sitePrefix[site] === '') {
